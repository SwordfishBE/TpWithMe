    ...
    private static void checkForUpdate() {
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                TpWithMe.LOGGER.debug("[TpWithMe] Update check returned HTTP {}.", response.statusCode());
                return;
            }

            Optional<String> latestVersion = extractLatestVersion(response.body());
            if (latestVersion.isEmpty()) {
                TpWithMe.LOGGER.debug("[TpWithMe] Update check returned no usable versions.");
                return;
            }
            ...
            if (isNewerVersion(newestVersion, currentVersion)) {
                TpWithMe.LOGGER.debug("[TpWithMe] New version available: {} (current: {})",
            ...
            TpWithMe.LOGGER.debug("[TpWithMe] Update check failed.", e);
        }
    }
